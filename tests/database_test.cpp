...
TEST(DbTest, AddAndGetMetadataValue) {
  Db db;
  {
    auto writer = db.CreateWriter();
    gendb::MetadataValueBuilder builder;
    builder.set_type(MetadataType::kSequence);
    builder.set_id(42);
    builder.set_int_value(123);
    builder.set_string_value("meta-info");
    std::vector<uint8_t> buffer = builder.Build();
    EXPECT_TRUE(writer.PutMetadataValue({MetadataType::kSequence, 42}, buffer).ok());
    writer.Commit();
  }
  {
    auto guard = db.SharedLock();
    gendb::MetadataValue metadata_value;
    EXPECT_TRUE(guard.GetMetadataValue({MetadataType::kSequence, 42}, metadata_value).ok());
    EXPECT_EQ(metadata_value.type(), MetadataType::kSequence);
    EXPECT_EQ(metadata_value.id(), 42);
    EXPECT_EQ(metadata_value.int_value(), 123);
    EXPECT_EQ(metadata_value.string_value(), "meta-info");
  }
}

TEST(DbTest, UpdateMetadataValueWithPatch) {
  Db db;
  {
    auto writer = db.CreateWriter();
    gendb::MetadataValueBuilder builder;
    builder.set_type(MetadataType::kSequence);
    builder.set_id(100);
    builder.set_int_value(555);
    builder.set_string_value("initial");
    std::vector<uint8_t> buffer = builder.Build();
    EXPECT_TRUE(writer.PutMetadataValue({MetadataType::kSequence, 100}, buffer).ok());
    writer.Commit();
  }
  {
    auto writer = db.CreateWriter();
    auto patch = gendb::MetadataValuePatchBuilder()
                     .set_int_value(777)
                     .set_string_value("updated")
                     .BuildPatch();
    EXPECT_TRUE(writer.UpdateMetadataValue({MetadataType::kSequence, 100}, patch).ok());
    writer.Commit();
  }
  {
    auto guard = db.SharedLock();
    gendb::MetadataValue metadata_value;
    EXPECT_TRUE(guard.GetMetadataValue({MetadataType::kSequence, 100}, metadata_value).ok());
    EXPECT_EQ(metadata_value.int_value(), 777);
    EXPECT_EQ(metadata_value.string_value(), "updated");
  }
}
